...
# GitHub Actions側の環境変数を優先する
DROP_THRESHOLD = float(
    os.getenv("DROP_THRESHOLD", "-3.0")
)

# Yahoo!ファイナンスへの連続アクセスを避ける
...
def find_stock_container(
    ...
    for _ in range(9):
        ...
        if (
            code in text
            and "%" in text
            and ROW_VALUE_PATTERN.search(text)
        ):
            return parent
...
def parse_source_updated_at(
    ...
    match = SOURCE_TIME_PATTERN.search(
        page_text
    )
    ...
    return f"{source_date} {source_time}"
...
def parse_ranking_page(
    ...
    for anchor in soup.find_all(
        ...
        container = find_stock_container(
            anchor,
            code
        )
        ...
        try:
            price = parse_number(
                value_match.group("price")
            )

            diff_yen = parse_number(
                value_match.group("diff")
            )

            diff_percent = parse_number(
                value_match.group("percent")
            )
